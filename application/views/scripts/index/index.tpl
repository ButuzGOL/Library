<?php if(count($this->books) > 0): $k = 0; if(count($this->books) == 1) $k = 1;?>
    <?php foreach ($this->books as $book): ?>
        <div class="post<?php echo $k; ?>">
            <a href="<?php echo $this->url(array('Id' => $book->id), 'book'); ?>" style="font-size:24px; font-weight:bold;"><?php echo $book->title; ?></a>  <?php if($this->auth->id): ?><img src="<?php echo $this->img . '/' . $book->bookmark; ?>.png" alt="Закладка" title="Закладка" style="cursor:pointer;" onclick="callAjaxBookmark('bookmark<?php echo $book->id; ?>', '<?php echo $book->id; ?>', <?php echo $this->baseUrl; ?>);" id="bookmark<?php echo $book->id; ?>" /><?php endif; ?> 
            <div class="text">
    ...
    <?php if($this->countpages > 1): ?>
        <?php if($this->page != 0): ?>
            <a href="<?php echo $this->url(array('pageId' => ($this->page - 1)), 'page'); ?>">Назад</a>
        <?php else: ?>
            <span>Назад</span>
        <?php endif; ?>
        <?php for ($i = 0; $i < $this->countpages; $i++): ?>
            <?php if($i == $this->page): ?>
                <span class="sp"><?php echo $i; ?></span>
            <?php else: ?>
                <a href="<?php echo $this->url(array('pageId' => $i), 'page'); ?>"><?php echo $i; ?></a>
            <?php endif; ?>
        <?php endfor; ?>
        
        <?php if($this->page != ($this->countpages - 1)): ?>
            <a href="<?php echo $this->url(array('pageId' => ($this->page + 1)), 'page'); ?>">Вперед</a>
        <?php else: ?>
            <span>Вперед</span>
        <?php endif; ?>
    <?php endif; ?>
