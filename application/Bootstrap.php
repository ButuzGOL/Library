...
class Bootstrap {
    ...
    public function run($config) {
        try {
            ...
            // Подключаем плагины
            require_once $this->_config->path->plugins . 'FlashMessenger.php';
            require_once $this->_config->path->plugins . 'CheckLogin.php';
            
            // Настройка front контроллера, указание базового URL, правил маршрутизации
            $front->setBaseUrl($this->_config->url->base)
                  ->setRouter($router)
                  ->throwexceptions(true)
                  ->registerPlugin(new FlashMessenger())
                  ->registerPlugin(new CheckLogin());
            ...
            // Перехват исключений 
            require($this->_config->path->system . 'Error.php');
    ...
    public function setView() {
        ...
        // Задание базового URL где лежат картинки
        $view->img = $this->_config->url->img;
        
        // Задание базового URL где лежат css файлы
        $view->css = $this->_config->url->css;
        
        // Задание базового URL где лежат js файлы
        $view->js = $this->_config->url->js;
        
        // Идентефицируем пользователя
        $view->auth = Zend_Auth::getInstance()->getIdentity();
        
        $Categories = new Categories();
        $view->categories = $Categories->selectCategories();
